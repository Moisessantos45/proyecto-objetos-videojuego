...
public class AcertijosLoader {
    ...
    private void cargarAcertijos(String rutaArchivo) {
        try {
            // Cargar desde classpath (funciona en JAR/EXE)
            String contenido;
            var inputStream = getClass().getClassLoader().getResourceAsStream(rutaArchivo);

            if (inputStream != null) {
                // Cargar desde classpath (JAR/EXE)
                contenido = new String(inputStream.readAllBytes());
                inputStream.close();
            } else {
                // Fallback: intentar cargar desde file system (desarrollo)
